...
// not works right now
export const getProposalVoteByAddress = (proposalId, voter) => {
  const url = `gov/proposals/${proposalId}/votes/${voter}`
  return get(url, {}).then(vote => {
    return vote
  }).catch(_e => {
    return null
  })
...
export const getProposalDepositByAddress = (proposalId, depositor) => {
  const url = `gov/proposals/${proposalId}/deposits/${depositor}`
  return get(url, {}).then(deposit => {
    return deposit
  })
}
